...
def do_delivery():
    d_s, d_a = does_sales.get(), does_annually.get()
    try:
        ...
        assert not (d_s and not clilist), (
            '若要生成业务员统计，必须选择客户分配表')
        assert d_s or d_a, '至少选择生成一项'
        f1_label.set('转换中')
        utils.log.clear()
        mailtable.handle(manifest, clilist, d_s, d_a)
        f1_label.set('转换完成')
        utils.log.show()
...
does_annually = tk.BooleanVar(value=False)
tk.Checkbutton(window, text='生成年度统计', variable=does_annually).pack()
